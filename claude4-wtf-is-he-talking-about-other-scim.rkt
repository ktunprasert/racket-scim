...
(struct scim-filter () #:transparent)
(struct scim-logical-expr (op left right) #:super scim-filter #:transparent)
(struct scim-comparison-expr (attr op value) #:super scim-filter #:transparent)
(struct scim-group-expr (expr) #:super scim-filter #:transparent)
(struct scim-not-expr (expr) #:super scim-filter #:transparent)
(struct scim-present-expr (attr) #:super scim-filter #:transparent)
...
(define scim-filter-parser
  (parser
   ...
   (error (lambda (tok-ok? tok-name tok-value start-pos end-pos)
            (error (format "SCIM filter parse error at ~a: unexpected ~a (~a)"
                          start-pos tok-name tok-value))))

   ;; Operator precedence (lowest to highest)
   (precs (left OR)
          (left AND)
          (right NOT)
          (left EQ NE GT GE LT LE CO SW EW PR))
   ...
   (grammar
    ...
    (comparison-expression
     [(attr-path EQ value) (scim-comparison-expr $1 'eq $3)]
    ...
    (attr-path
     [(IDENTIFIER) (scim-attr-path $1 #f #f)]
...
(define (scim-filter->data filter)
  (match filter
    ...
    [(scim-comparison-expr attr op value)
     `((type . "comparison")
       (attribute . ,(attr-path->data attr))
       (operator . ,op)
       (value . ,value))]

    [(scim-present-expr attr)
     `((type . "present")
       (attribute . ,(attr-path->data attr)))]

    [(scim-not-expr expr)
     `((type . "not")
       (expression . ,(scim-filter->data expr)))]

    [(scim-group-expr expr)
     `((type . "group")
       (expression . ,(scim-filter->data expr)))]))
  ...
  (match attr-path
    [(scim-attr-path name sub-attr filter)
     `((name . ,(symbol->string name))
       (sub-attribute . ,(if sub-attr (symbol->string sub-attr) null))
       (filter . ,(if filter (scim-filter->data filter) null)))]))
    ...
    (cond
      ...
      [(list? data)
       (if (and (pair? data) (pair? (car data)))
           ...
           (format "{~a}"
                   (string-join
                    (map (lambda (p) (format "\"~a\":~a" (car p) (data->json (cdr p))))
                         data) ","))
           ;; Array
  ...
  (match filter
    [(scim-comparison-expr (scim-attr-path name sub #f) op value)
     (format "query.Where(\"%s %s ?\", \"%s\", %s)"
             (if sub (format "%s.%s" name sub) (symbol->string name))
             (op->sql op)
             (value->go-literal value))]

    [(scim-logical-expr 'and left right)
     (format "(%s) AND (%s)"
             (scim-filter->go-query left)
             (scim-filter->go-query right))]
